...
#initialisation de la grille en suivant les règle du jeu dodo pour n'importe quelles tailles de grilles 
...
#Initialisation de dodo pour un jeu en mode corners et une taille de grille de 3 (cas du serveur avec petite grille)
#!   /!\ le dico_conversion contient des clés de type Cell_hex et des valeurs de type Cell_mat /!\
def init_grille_dodo_corners() -> Tuple[Grid, Dict, DirectionJeu]: #!OK
    ...
    for key in dico_conversion.keys(): #initialisation de la grille : toutes les cases qui ne sont pas en clé du dico n'existent pas
        ...
        if key[0]+key[1] >= taille_grille: #on remplit la grille avec les pions bleu
            grille[x_cell][y_cell] = BLEU
        elif key[0]+key[1] <= -taille_grille: #on remplit la grille avec les pions rouges
            grille[x_cell][y_cell] = ROUGE
        else:
            grille[x_cell][y_cell] = EMPTY #on remplit la grille avec les cases vides
...
def init_grille_dodo_one_ligne() -> Tuple[Grid, Dict, DirectionJeu]: #!OK
    ...
    for key in dico_conversion.keys(): #initialisation de la grille : toutes les cases qui ne sont pas en clé du dico n'existent pas
        x_cell, y_cell = dico_conversion[key] #! type de dico_conversion[key] : Cell_mat
        if key[0]+key[1] >= taille_grille-2: #on remplit la grille avec les pions bleu
            grille[x_cell][y_cell] = BLEU
        elif key[0]+key[1] <= -taille_grille+2: #on remplit la grille avec les pions rouges
            grille[x_cell][y_cell] = ROUGE
        else:
            grille[x_cell][y_cell] = EMPTY #on remplit la grille avec les cases vides
...
def est_legal(grille : Grid,
              dico_conversion : Dict, direction: DirectionJeu,
              action : ActionDodo, joueur : Player) -> bool:
...
def liste_coup_legaux(grille: Grid,
                      dico_conversion: Dict,
                      direction: DirectionJeu,
                      joueur: Player) -> List[ActionDodo]:
...
def score(grille, dico_conversion, direction, joueur_max) -> Score:
    ...
    if joueur_max == ROUGE:
        return ( -len(liste_coup_legaux(grille, dico_conversion, direction, BLEU))
            + len(liste_coup_legaux(grille, dico_conversion, direction, ROUGE)) )
    return (-len(liste_coup_legaux(grille, dico_conversion, direction, ROUGE))
            +len(liste_coup_legaux(grille, dico_conversion, direction, BLEU)) )
...
def trier_actions(grid,
                  dico_conversion,
                  direction,
                  liste_actions:List[ActionGopher],
                  player_max:Player) -> List[ActionGopher]:
...
@memoize
def alpha_beta_dodo(grid : Grid,
                    dico_conversion,
                    direction,
                    player_max : Player,
                    depth, alpha, beta) -> Tuple[Score, ActionGopher]:
    ...
    if player_max == ROUGE:
        best_value = -INF
        best_action = None
        for action in trier_actions(grid, dico_conversion, direction,
                                    liste_coup_legaux(grid, dico_conversion, direction, ROUGE),
                                    player_max):
            new_grid = play_action(grid, dico_conversion, action, ROUGE)
            new_value, _ = alpha_beta_dodo(new_grid,dico_conversion,direction, BLEU,
                                           depth - 1, alpha, beta)
    ...
    for action in trier_actions(grid, dico_conversion, direction,
        ...
        new_value, _ = alpha_beta_dodo(new_grid, dico_conversion, direction, ROUGE,
                                       depth - 1, alpha, beta)
